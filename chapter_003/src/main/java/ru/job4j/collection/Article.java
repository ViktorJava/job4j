...
public class Article {
    ...
    public static boolean generateBy(String origin, String line) {
        ...
        for (String key : bigLineArray) {
            // Выполняем лямбду если ключ существует в карте.
            if (tempMap.computeIfPresent(key, (k, v) -> v + 1) == null) {
                // Иначе добавляем пару.
                tempMap.putIfAbsent(key, 1);
            }
        }
        // Слова из newLineArray сверяем с ключами заполненной карты.
        for (String key : newLineArray) {
            // Выполняем лямбду если ключ существует в карте.
            if (tempMap.computeIfPresent(key, (k, v) -> v - 1) == null) {
                // Иначе записываем новый ключ со значением -1.
                tempMap.putIfAbsent(key, -1);
            }
            // Проверяем значение текущего ключа на -1.
